...
function AdminDashboard() {
  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      if (
        walletStatsResult.status === "fulfilled" &&
        walletStatsResult.value &&
        walletStatsResult.value.data
      ) {
      ...
      if (
        revenueStatsResult.status === "fulfilled" &&
        revenueStatsResult.value &&
        revenueStatsResult.value.data
      ) {
      ...
      if (listingsResult.status === "fulfilled") {
        ...
        if (listings && listings.length > 0) {
          ...
          const sellCount = listings.filter(
          ...
          listings.forEach((item) => {
            ...
            if (
              ...
              buildingType.includes("co-working") ||
              buildingType.includes("coworking") ||
              buildingType.includes("office")
            ) {
              propertyTypes["co-working"]++;
            } else if (
              buildingType.includes("hotel") ||
              buildingType.includes("lodge") ||
              buildingType.includes("resort")
            ) {
              propertyTypes.hotel++;
            } else if (
              buildingType.includes("house") ||
              buildingType.includes("villa") ||
              buildingType.includes("duplex") ||
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-6">
        ...
        <DashboardStats
          ...
          showDate={true}
        />
        {/* Make the last card span 2 columns only on large screens to avoid wrapping on tablet */}
        <DashboardStats
          loading={loading}
